...
export default function CrawlWithTokenPage() {
  ...
  const handleCreateToken = async () => {
    try {
      const response: AxiosResponse<{ token: string; error: string }> =
        await axios.post(`${process.env.NEXT_PUBLIC_SERVICE_BASE_URL}/token`, {
          ...
          <div className="bg-blue-50 rounded-lg border border-gray-200 p-4 shadow-sm">
            ...
            {kyaToken ? (
              <>
                ...
                <div className="mt-6">
                  <h2 className="mb-4 text-xl font-semibold text-gray-900">
                    Step 2: Inspect the created token
                  </h2>
                ...
                <div className="mt-8">
                  <h2 className="mb-4 text-xl font-semibold text-gray-900">
                    Step 3: Select website to crawl
                  </h2>
                  <div className="mt-6">
                    <CrawlSearchLog
                      pageRoute={"/token"}
                      skyfireKyaToken={kyaToken}
                      onAwsUrlChange={setSelectedUrl}
                    />
                  </div>
                </div>
              </>
